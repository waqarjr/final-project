...
export const Header = () => {
  ...
  const cartProducts = async () => {
    try {
      ...
      const response = await axios.post(`http://localhost:4000/cart-product`, { email: email }, {
        withCredentials: true
      });
      setFetchData(response.data);
    } catch (err) {
      console.error("Header failed to load cart products", err);
    }
  };
  ...
  fetchData.forEach((item) => {
    if (item.productDetails) {
      const price = item.productDetails.price_discount && item.productDetails.price_discount < item.productDetails.price
        ? item.productDetails.price_discount
  ...
  const delCart = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:4000/del-cart/${id}`, {
        withCredentials: true
      });
      if (response.data.a) {
        removeCart();
    ...
    <>
      ...
      <nav className="border-b bg-lightyellow">
        <div className="w-full px-2 sm:px-8 lg:px-20">
          <div className="flex items-center justify-between h-[90px]">
            ...
            <div className="sm:flex flex-1 md:w-full lg:max-w-md px-4 ">
              <div className="relative md:w-full max-w-sm md:mx-0 mx-auto">
                <FontAwesomeIcon icon={faMagnifyingGlass} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                <input type="search" placeholder="Search Product" className="w-full pl-10 pr-2 sm:pr-4 sm:py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-600 focus:border-transparent" />
              </div>
            </div>

            <div className="flex items-center space-x-2 md:space-x-4">
              <div className="relative">
                <div className="cursor-pointer" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} onClick={() => { setToogle(!toogle) }} >
                ...
                {sign ? (
                  <>
                    <div className={`absolute right-0 mt-2 w-64 bg-white rounded-md shadow-lg z-50 transition-all duration-300 ${isHovered || toogle ? ' opacity-100' : 'opacity-0 '} `} >
                      <div className="p-4">
                          ...
                          {fetchData.map(item => {
                            ...
                            return (
                              <div key={item._id} className="flex items-start border-b pb-3">
                                ...
                                <button className="text-gray-400 hover:text-gray-600 ml-2" onClick={() => delCart(item._id)}>
                                  <span>&times;</span>
                                </button>
                              </div>
                            );
                        ...
                        <div className="mt-4">
                          ...
                          <div className="flex space-x-2">
                            <Link to="/viewcart" className="bg-emerald text-white hover:bg-white border hover:text-emerald border-emerald duration-200 px-4 py-2 rounded w-1/2 text-sm font-medium text-center">
                              View Cart
                            </Link>
                            <Link to='/checkout' className="bg-emerald text-white hover:bg-white border hover:text-emerald border-emerald duration-200 px-4 py-2 rounded w-1/2 text-sm font-medium text-center flex items-center justify-center transition-colors">
                              Checkout
                            </Link>
                          </div>
                        </div>
          ...
          <div id="mobile-menu" className={`lg:hidden w-full overflow-hidden transition-all duration-300 ease-in-out text-center ${isMobileMenuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'} bg-lightyellow`} >
            ...
            <Link to="/account" className="block py-2 text-gray-800 font-semibold hover:bg-blue-200">Account</Link>
