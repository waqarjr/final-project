...
export const Product = () => {
  ...
  const fetchReviews = async (productId) => {
    try {
      const response = await axios.get(`http://localhost:4000/getreviews/${productId}`);
  ...
  const fetchData = async (productId) => {
    try {
      const response = await axios.get(`http://localhost:4000/read-update-product/${productId}`);
      setTitle(response.data.title);
      setKeywords(response.data.keywords);
      setPrice(response.data.price);
      setLong_Description(response.data.long_description);
  ...
  const fetchMultiple = async (productId) => {
    try {
      const response = await axios.get(`http://localhost:4000/read-mul-image-product/${productId}`);
      setMulImage(response.data);
    } catch (err) {
      console.error("Failed to load multiple product images", err);
    }
  };
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      reviews: Yup.string().required("Please write your idea about this product").min(10, "Text must be at least 10 characters long"),
    }),
    onSubmit: async (values, { resetForm }) => {
      if (rating === 0) {
        setEror("Please select a rating");
        return;
      }
      setEror("");
      setLoading(true);
  ...
  const toCart = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:4000/cartitems', {
        ...
        withCredentials: true // Authenticated request
      });
    ...
    <>
          ...
          <div className="grid gap-12 lg:grid-cols-2">
            <div className="space-y-4">
              <div className="relative overflow-hidden rounded-lg bg-gray-100 flex items-center justify-center">
                <img src={image} className="w-[400px] object-contain p-4" alt={title} />
              </div>
              <div className="grid grid-cols-4 gap-4">
                {mulImage.map((value, index) => (
                  <button key={index} className="relative overflow-hidden rounded-lg border border-gray-300 bg-gray-100">
                    <img src={value.images} className="h-full w-full object-contain" alt="" />
                  </button>
                ))}
              </div>
            </div>
      ...
      <div className='my-10'>
        ...
        <div className={`${toggle ? "hidden" : ""} max-w-7xl mx-auto border-2 rounded border-gray-200`}>
          <div className='m-3' dangerouslySetInnerHTML={{ __html: long_description }}></div>
        </div>
        <div className={`${toggle ? "" : "hidden"} max-w-7xl mx-auto border-2 rounded border-gray-200`}>
          <div className='m-3'>
            ...
            <div className='grid grid-cols-[60%_auto]'>
              ...
              <Link to="/signin" className={`flex justify-center text-emerald ${localStorage.getItem('isSigup') ? 'hidden' : ''}`}>Sign in / Sign up</Link>
              <div className={`mx-auto ${localStorage.getItem('isSigup') ? '' : 'hidden'}`}>
                <form onSubmit={formik.handleSubmit}>
                    ...
                    <div className="flex flex-col items-center justify-center pb-4">
                      <div className="flex items-center space-x-1">
                        {[...Array(5)].map((_, index) => {
                          ...
                          return (
                            <button key={index} onClick={() => setRating(starNumber)} className="focus:outline-none" type="button">
                              <FontAwesomeIcon icon={faStar} className={`fa fa-star text-3xl transition-colors duration-200 ${starNumber <= rating ? 'text-yellow-400' : 'text-gray-300 hover:text-yellow-300'}`} />
                            </button>
                          );
                        })}
                      </div>
                      <span className='text-red-500'>{eror}</span>
                    </div>
                  ...
                  <button type="submit" disabled={loading} className="rounded-sm px-4 py-2 hover:bg-emerald text-emerald hover:text-white border-emerald border-2 disabled:opacity-50">
