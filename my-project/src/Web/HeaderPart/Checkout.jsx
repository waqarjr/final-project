...
export const Checkout = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      phone: Yup.string().required("Enter your phone number").min(15, "Enter a valid phone number"),
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        formData.append("amount", a); // Sent for UI comparison, verified on server
        ...
        const response = await axios.post("http://localhost:4000/finalorder", formData, {
          withCredentials: true
        });
        ...
        if (response.data.abc) {
          ...
          Swal.fire({
            text: `${response.data.abc}`,
            icon: "success"
          });
          navigate('/');
        } else {
          Swal.fire({
            text: response.data.error || "Order placement failed.",
            icon: "error"
          });
        }
      } catch (err) {
        const errorMsg = err.response?.data?.error || err.response?.data?.message || "Failed to place order. Check stock availability.";
        Swal.fire({
          title: "Order Failed!",
          text: errorMsg,
          icon: "error"
    ...
    <>
      ...
      <div className='my-4'>
        <p className='text-gray-400 max-w-7xl mx-auto'>
          ...
          <span className='hover:text-black cursor-pointer' onClick={() => { navigate('/viewcart') }}>View Cart</span> &nbsp; &gt; &nbsp;
          <span className='hover:text-black cursor-pointer'>Check Out</span>
        </p>
      </div>
      <hr />
            ...
            <div className="lg:w-2/3">
              ...
              <div className="mb-4">
                ...
                {formik.touched.address && formik.errors.address && (
                  <span className="text-rose-500 text-xs block mt-1">{formik.errors.address}</span>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4">
                <div>
                  ...
                  {formik.touched.postcode && formik.errors.postcode && (
                    <span className="text-rose-500 text-xs block mt-1">{formik.errors.postcode}</span>
                  )}
                </div>
                <div>
                  ...
                  {formik.touched.city && formik.errors.city && (
                    <span className="text-rose-500 text-xs block mt-1">{formik.errors.city}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="lg:w-1/3">
              <div className="border border-gray-200 rounded p-6">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-white border border-emerald text-emerald py-3 rounded hover:bg-emerald hover:text-white transition-colors disabled:opacity-50"
                >
