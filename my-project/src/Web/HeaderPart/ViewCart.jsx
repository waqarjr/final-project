...
export const ViewCart = () => {
  ...
  const quantityValue = async (id, count) => {
    ...
    try {
      const email = localStorage.getItem("userEmail");
      if (email != null) {
        const response = await axios.post('http://localhost:4000/chnagequantity', {
          email: email,
          id: id,
          quantity: qty
        }, {
                      ...
                      <td className="py-2">
                        <div className="w-24 flex items-center rounded-md border border-emerald">
                          <input
                            ...
                            min="1"
                            value={item.quantity}
                            onChange={(e) => { const newValue = e.target.value; quantityValue(item._id, newValue); }}
                            className="w-full p-2 border border-gray-300 rounded text-center"
                          />
                        </div>
                      </td>
                      <td className="py-2 text-emerald font-medium">{item.quantity * price}</td>
                      <td className="py-2">
