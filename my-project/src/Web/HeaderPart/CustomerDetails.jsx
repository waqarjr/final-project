...
export const CustomerDetail = () => {
  ...
  const customerData = async (id) => {
    setLoading(true);
    try {
      const response = await axios.post(`http://localhost:4000/siglecus-data/${id}`, {}, {
      ...
      Swal.fire({
        title: "Error!",
        text: err.response?.data?.error || "Failed to load order details.",
        icon: "error"
      });
    } finally {
      setLoading(false);
  ...
  const customerProduct = async (id) => {
    try {
      const response = await axios.post(`http://localhost:4000/singcus-product/${id}`, {}, {
        withCredentials: true
      });
  ...
  fetchData.forEach((item) => {
    const price = item.price_discount && item.price_discount < item.price ? item.price_discount : item.price;
      ...
      <div className="max-w-7xl mx-auto">
        <div className="my-8">
          <p className="text-gray-400">
            ...
            <span className="hover:text-black cursor-pointer" onClick={() => navigate('/account')}>Account</span> &nbsp; &gt; &nbsp;
            <span className="hover:text-black cursor-pointer">Order Details</span>
          </p>
        </div>
        <hr />
        ...
        <div className="p-2">
          {loading ? (
            <div className="text-center py-8 font-medium text-gray-500">Loading details...</div>
          ) : (
            <div className="max-w-auto mx-auto p-4 bg-white">
              ...
              <div className="grid grid-cols-2">
                <div></div>
                <div>
                  <h1 className="text-lg font-bold text-gray-800 mb-4 capitalize py-4 border-b-2">Shopping Cart Total</h1>
                  <div className="flex items-center justify-between *:font-semibold *:text-gray-800">
                    <span>Total (excl. shipping)</span> <span>Rs. {total}</span>
                  </div>
                </div>
              </div>
            </div>
